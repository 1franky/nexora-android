...
interface CreditCardApi {
    ...
    @POST("credit-cards")
    suspend fun createCreditCard(@Body request: CreateCreditCardRequest): CreditCard

    @GET("credit-cards/{id}")
    suspend fun getCreditCard(@Path("id") id: String): CreditCard

    @POST("credit-cards/{id}/purchases")
    suspend fun purchase(@Path("id") id: String, @Body request: CreditCardPurchaseRequest): Transaction

    @POST("credit-cards/{id}/payments")
    suspend fun pay(@Path("id") id: String, @Body request: CreditCardPaymentRequest): TransferResult
}
